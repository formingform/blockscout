edefmodule Indexer.Fetcher.PlatonAppchain.EpochL2Validator do
  ...
  @spec handle_continue(map(), EpochL2Validator, atom()) :: {:noreply, map()}
  def handle_continue(
        %{
          l2_validator_contract_address: l2_validator_contract_address,
          l2_rpc_arguments: l2_rpc_arguments,
      ...
      when calling_module in [EpochL2Validator] do
    ...
    if next_epoch_block == 0 || latest_block >= next_epoch_block do
      ...
      # 设置验证人类型
      epoch_validators = Enum.map(epoch_validators, fn(validator) ->  Map.put(map, :status, PlatonAppchain.validator_status()[:Active]) end)

      PlatonAppchain.log_validators(all_candidates, "Active", "epoch", epoch, last_block, "L2")

      # 更新所有质押人
      all_candidates = L2StakeHandler.getAllValidators()
      # 设置验证人类型
      all_candidates = Enum.map(all_candidates, fn(candidate) ->  Map.put(map, :stats, PlatonAppchain.validator_status()[:Candidate]) end)

      PlatonAppchain.log_validators(all_candidates, "Candidate", "epoch", epoch, last_block, "L2")
    end
