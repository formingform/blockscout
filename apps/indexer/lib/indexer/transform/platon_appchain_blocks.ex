defmodule Indexer.Transform.PlatonAppchainBlocks do
  @moduledoc """
  ...
  def signer(block) when is_map(block) do
    ...
    extra_data = decode(block.extra_data)

    extra = binary_part(extra_data, 0,32)
    signature = binary_part(extra_data, 32, 65)

    block = %{block | extra_data: extra}
    signature_hash = signature_hash(block)

    recover_pub_key(signature_hash, signature)
  end
  ...
  def recover_pub_key(signature_hash, signature) do
    <<
      r::bytes-size(32),
      s::bytes-size(32),
      v::integer-size(8)
